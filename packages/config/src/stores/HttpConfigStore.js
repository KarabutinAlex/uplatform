...
class HttpConfigStore {
  ...
  fetch() {
    return new Promise((resolve, reject) => {
      try {
        const createRequest = this.url.match(/^https:/)
          ? https.request
          : http.request;

        const request = createRequest(this.url, (response) => {
          const chunks = [];
        ...
        request.on('error', (error) => {
          reject(new Error(`Can't load config from "${this.url}": ${error.message}`));
        });

        request.end();
      } catch (error) {
        reject(new Error(`Can't send a request to "${this.url}": ${error.message}`));
      }
    });
  }
}
